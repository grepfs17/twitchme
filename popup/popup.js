document.addEventListener('DOMContentLoaded', () => {
  ...
  async function addChannel() {
    ...
    chrome.runtime.sendMessage({ type: 'ADD_CHANNEL', channel: name }, (data) => {
      renderView(data)
    })
  }
  ...
  function renderChannels(data) {
    ...
    channels.forEach(ch => {
      ...
      removeBtn.addEventListener('click', () => {
        chrome.runtime.sendMessage({ type: 'REMOVE_CHANNEL', channel: ch.name }, (d) => {
          renderView(d)
        })
      })
  ...
  function updateStatusText(data) {
    const channels = data.channels || []
    const liveCount = Object.values(data.liveChannels || {}).filter(Boolean).length
